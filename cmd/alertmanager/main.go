...
func (self *BHook) Fire(level blog.LevelType, message ...interface{}) {
	fmt.Println(message)
}
...
func init() {
	...
	err := blog.NewWriterFromConfigAsFile("config.xml")
	if nil != err {
	...
	hook := new(BHook)
	blog.SetHook(hook)
	blog.SetHookLevel(blog.INFO)
	blog.SetColored(true)
	blog.Debug("start alertmanager")
}
